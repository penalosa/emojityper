...
emoji.forEach(e => {
  ...
  name.forEach(n => {
    mapping[n.trim().toLowerCase()] = mapping[n.trim().toLowerCase()] || e;
  });
  if (e.keywords) {
    let keywords = e.keywords.split("|");
    keywords.forEach(k => {
      mapping[k.trim().toLowerCase()] = mapping[k.trim().toLowerCase()] || e;
    });
  }
});
console.log("window.emoji = ", mapping);
